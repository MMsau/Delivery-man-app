...
class InfoCard extends StatelessWidget {
  ...
  const InfoCard({Key? key, required this.orderId, required this.title, required this.image, required this.name, required this.address, this.orderAmount, required this.phone,
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            showButton ? Row(children: [
              ...
              TextButton.icon(
                ...
                icon: Icon(Icons.directions, color: Theme.of(context).secondaryHeaderColor, size: 20),
                label: Text(
                  'direction'.tr,
                  style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall, color: Theme.of(context).primaryColor),
                ),
              ...
              TextButton.icon(
                onPressed: () async {
                  String url ='http://contact.mastermovers.ph/index.php?name=MasterMovers+Driver&email=Reported:+$name&orderNumber=$orderId';
                  if (await canLaunchUrlString(url)) {
                ...
                icon: Icon(Icons.report_problem, color: Theme.of(context).hintColor, size: 20),
                label: Text(
                  'Report'.tr,
